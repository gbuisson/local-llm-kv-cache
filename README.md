...
Operational events are emitted as compact JSON. Session identifiers、rendered prompt、token ID 数组和 manifest 内容都不会写入日志；`session_ref` 是可用于关联的截断 SHA-256。Shared-prefix 事件只记录候选 token 数和验证后的 LCP 长度等计数。完整 golden 是当前请求的 token prefix 时才允许 restore，并记录 `shared_prefix_candidate_restored`；响应 metadata 返回后才记录 `shared_prefix_effective_hit`（`cached_tokens > 0`）、`shared_prefix_rejected_by_llama`（`cached_tokens == 0`）或 `shared_prefix_effectiveness_unknown`（上游没有提供该计数）。候选在内部发生分歧时不 restore，而是记录 `shared_prefix_overlap_discovered`，cold 完成后 best-effort seed 严格验证的 `request_tokens[:verified_lcp]`。这样 memory、时间戳等动态 suffix 不会阻止稳定的较短 golden 被复用。若单 slot 被未保存的 tool-call 会话占用，seed 保留在有界内存队列中，并在后续请求完成后重试。非 `default` scope 只接受带有效 manifest 的 snapshot；无 manifest 的 legacy exact-prefix fallback 仅保留给历史 `default` scope。Golden seed 会重新扫描有效 manifest；若相同 namespace、scope 和 exact token IDs 已发布，则记录 `prefix_seed_duplicate_skipped`，不会再次 prefill 或生成随机副本。
